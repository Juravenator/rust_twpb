...
fn test_leb128_decode() {
    ...
    let val = ::twpb::decoder::leb128_i32(
        [
            0xBB, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x01,
        ]
        .into_iter(),
    )
    .unwrap();
    ...
    let val = ::twpb::decoder::leb128_i64(
        [0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x01].into_iter(),
    )
    .unwrap();
...
fn test_leb128_decode_overflows() {
    // encoded i64::MIN
    let result = ::twpb::decoder::leb128_i32(
        [0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x01].into_iter(),
    )
    .unwrap_err();
    assert_eq!(result, ::twpb::decoder::DecodeError::TooLargeVarint);
    let result = ::twpb::decoder::leb128_u32(
        [0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x01].into_iter(),
    )
    .unwrap_err();
    assert_eq!(result, ::twpb::decoder::DecodeError::TooLargeVarint);
