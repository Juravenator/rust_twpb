...
fn test_tags_overflow() {
    let mut buffer = Vec::<u8>::new();
    let result = ::twpb::encoder::tag(&mut buffer, &u32::MAX, &1).unwrap_err();
    assert_eq!(result, ::twpb::encoder::EncodeError::OverflowError);
    let result = ::twpb::encoder::tag(&mut buffer, &1, &u8::MAX).unwrap_err();
    assert_eq!(result, ::twpb::encoder::EncodeError::OverflowError);
}