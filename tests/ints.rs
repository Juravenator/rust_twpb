...
fn test_ints_encode_correctly_if_msb_set() {
    let mut buffer = Vec::<u8>::new();
    let bytes_written = ::twpb::encoder::uint64(&mut buffer, &(0x80_00_00_00_00_00_00_00)).unwrap();
    assert_eq!(bytes_written, 10);
    assert_eq!(buffer, [0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x01]);
    let result = ::twpb::decoder::uint64(buffer.iter(), "").unwrap();
    assert_eq!(result, 0x80_00_00_00_00_00_00_00);

    // same but for signed integers (zigzag encoding)
    let mut buffer = Vec::<u8>::new();
    let bytes_written = ::twpb::encoder::sint64(&mut buffer, &(-0x80_00_00_00_00_00_00_00)).unwrap();
    assert_eq!(bytes_written, 10);
    assert_eq!(buffer, [0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x01]);
    let result = ::twpb::decoder::sint64(buffer.iter(), "").unwrap();
    assert_eq!(result, -0x80_00_00_00_00_00_00_00);
}
...
fn test_ints_overflow() {
    let mut buffer = Vec::<u8>::new();
    let bytes_written = ::twpb::encoder::sint64(&mut buffer, &(-9223372036854775808 as i64)).unwrap();
    assert_eq!(bytes_written, 10);
    let result = ::twpb::decoder::sint32(buffer.iter(), "").unwrap_err();
