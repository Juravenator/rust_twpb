...
pub fn string<'a, const SIZE: usize, I>(
    mut bytes: I,
    field_name: &'static str,
) -> Result<heapless::String<SIZE>, DecodeError>
where
    I: Iterator<Item = u8>,
{
    ...
    for _ in 0..bufsize as usize {
        match bytes.next() {
            Some(byte) => strbuf
                .push(byte)
                .map_err(|_| DecodeError::UnexpectedEndOfBuffer)?,
...
pub fn bytes<'a, const SIZE: usize, I>(
    mut bytes: I,
    field_name: &'static str,
) -> Result<heapless::Vec<u8, SIZE>, DecodeError>
where
    I: Iterator<Item = u8>,
{
